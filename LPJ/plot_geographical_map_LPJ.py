...
explist=('sba0080',)
varlist=('agb_tree','vegc','agb')


typelist=('timmean',) #'ACyL10')
#typelist=('ACyL10',)
...
##          print(np.shape(data))
            ...
            #print(dims)
            if dims==4:
                print("4 dimensions detected - error!")
            ...
            if type.find('AC') != -1:
                ...
                vmin=0
                vmax=1
                data_clipped = np.clip(data2, vmin, vmax)
                ...
                m.pcolormesh(x, y, data2/1000, cmap='viridis', shading='auto')
                cbar = plt.colorbar(label='', orientation='horizontal', pad=0.05, shrink=0.75)
                cbar.ax.tick_params(labelsize=fontsize_ticks)  # Change font size here
                #cbar.formatter.set_powerlimits((0,0))
                #cbar.formatter.set_useMathText(True)
                #cbar.formatter.labelsize=fontsize_ticks
                cbar.set_label(varname + ' [kg/m2]', fontsize=fontsize_labels)
                        
            #plt.show()
           
            figname=exp+'_'+var+typestring+'_geographical_map.png'
